      ...
      <main className="flex-1 overflow-y-auto p-10">
        ...
        {activeTab === 'history' && (
          <div className="space-y-6">
            <div className="relative"><Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" /><input type="text" placeholder="Buscar en historial..." className="w-full pl-14 pr-6 py-5 rounded-2xl bg-white border-2 border-slate-100 outline-none focus:border-blue-600 font-black" value={searchId} onChange={(e) => setSearchId(e.target.value)} /></div>
        ...
        {activeTab === 'finance' && (
          <div className="bg-slate-900 p-16 rounded-[4rem] shadow-2xl text-white relative overflow-hidden">
            <TrendingUp className="absolute top-0 right-0 w-[400px] h-[400px] opacity-5 rotate-12" />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16 relative z-10">
              {[{ label: 'Empresa (60%)', val: finance.empresa }, { label: 'Productores (30%)', val: finance.productores }, { label: 'Vendedores (10%)', val: finance.vendedores }].map(({ label, val }) => (
                <div key={label}><p className="text-blue-400 font-black uppercase tracking-widest text-xs mb-4">{label}</p><h4 className="text-6xl font-black tracking-tighter">C${val.toFixed(2)}</h4></div>
              ))}
            </div>
          </div>
        )}
